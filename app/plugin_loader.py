...
def load_plugin(plugin_group ='trading-signal.plugins', plugin_name='default_plugin'):
    print(f"Attempting to load plugin: {plugin_name}")
    try:
        entry_point = pkg_resources.get_entry_map('trading-signal', plugin_group)[plugin_name]
...
def get_plugin_params(plugin_name):
    ...
    try:
        entry_point = pkg_resources.get_entry_map('trading-signal', 'trading-signal.plugins')[plugin_name]
        plugin_class = entry_point.load()
